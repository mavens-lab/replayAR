...
public:
    struct FilePose {
            ...
            float qz = 0;
            float qw = 0;
            FilePose(float tstamp = 0, float x = 0, float y = 0, float z = 0, float qx = 0, float qy = 0, float qz = 0, float qw = 0)
                : tstamp(tstamp), x(x), y(y), z(z), qx(qx), qy(qy), qz(qz), qw(qw) {}
        };
    struct Options
    {
         //added for offline pose
        std::queue<FilePose> poses0;
        FilePose initialPose;
        std::queue<FilePose> poses1; //additional copy of poses
        std::wstring poseFile;
        
        bool firstTransform = true;
        //flaag for if first frame passed
        bool flag = false;

        // flags from args
        bool write_pose = false;
        bool render_replay = true;
        std::wstring write_outputpose_file;
        ...
        winrt::Windows::Graphics::Holographic::HolographicStereoTransform fixedTransform;
        winrt::Windows::Graphics::Holographic::HolographicStereoTransform projTransform;
        winrt::Windows::Foundation::Rect viewPort;
        winrt::Windows::Perception::Spatial::SpatialCoordinateSystem coordSystemFixed = NULL;
...
public:
    ...
    winrt::Windows::Perception::Spatial::SpatialBoundingFrustum ConvertPoseToFrustum(SampleRemoteApp::FilePose nextPose);

    winrt::Windows::Foundation::Numerics::plane XMVectorToPlane(DirectX::XMVECTOR inputPlane);
    
    winrt::Windows::Graphics::Holographic::HolographicStereoTransform ConvertPoseToStereoTransform(SampleRemoteApp::FilePose nextPose);
...
private:
    ...
    std::unique_ptr<SpinningCubeRenderer> m_spinningCubeRenderer1;
    std::unique_ptr<SpinningCubeRenderer> m_spinningCubeRenderer2;
    std::vector<std::unique_ptr<SpinningCubeRenderer>> m_spinningCubeRendererVector = {};
