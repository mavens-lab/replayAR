...
winrt::Windows::Perception::Spatial::SpatialBoundingFrustum SampleRemoteApp::ConvertPoseToFrustum(SampleRemoteApp::FilePose nextPose) {
    auto position = DirectX::XMFLOAT3(nextPose.x, nextPose.y, nextPose.z);
    auto orientation = DirectX::XMFLOAT4(nextPose.qx, nextPose.qy, nextPose.qz, nextPose.qw);

    // DirectX::XMMATRIX for pose
    auto viewMatrix = DirectX::XMMatrixAffineTransformation(
        DirectX::XMVectorSet(1.0f, 1.0f, 1.0f, 0.0f),  // Scale
        DirectX::XMVectorZero(),                        // Rotation origin
        DirectX::XMQuaternionNormalize(DirectX::XMLoadFloat4(&orientation)),  // Orientation
        DirectX::XMLoadFloat3(&position)                 // Position
    );

    // Convert to DirectX::BoundingFrustum
    float fov = DirectX::XMConvertToRadians(64.69f);
    float aspectRatio = 16.0f / 9.0f;
    float nearClip = 0.1f;
    float farClip = 20.0f;
    auto projectionMatrix = DirectX::XMMatrixPerspectiveFovLH(fov, aspectRatio, nearClip, farClip);
    auto viewProjectionMatrix = DirectX::XMMatrixMultiply(viewMatrix, projectionMatrix);

    DirectX::BoundingFrustum frustum;
    DirectX::BoundingFrustum::CreateFromMatrix(frustum, viewProjectionMatrix);

    // Convert from DirectX::BoundingFrustum to SpatialBoundingFrustum
    DirectX::XMVECTOR nearPlane, farPlane, rightPlane, leftPlane, topPlane, bottomPlane;
    frustum.GetPlanes(&nearPlane, &farPlane, &rightPlane, &leftPlane, &topPlane, &bottomPlane);

    SpatialBoundingFrustum spatialFrustum;

    // Convert XMVectors to Planes
    spatialFrustum.Near = XMVectorToPlane(nearPlane);
    spatialFrustum.Far = XMVectorToPlane(farPlane);
    spatialFrustum.Bottom = XMVectorToPlane(bottomPlane);
    spatialFrustum.Top = XMVectorToPlane(topPlane);
    spatialFrustum.Left = XMVectorToPlane(leftPlane);
    spatialFrustum.Right = XMVectorToPlane(rightPlane);

    return spatialFrustum;
}

winrt::Windows::Foundation::Numerics::plane SampleRemoteApp::XMVectorToPlane(DirectX::XMVECTOR inputPlane) {
    DirectX::XMFLOAT3 normal;
    normal.x = DirectX::XMVectorGetX(inputPlane);
    normal.y = DirectX::XMVectorGetY(inputPlane);
    normal.z = -1*(DirectX::XMVectorGetZ(inputPlane));
    float distance = DirectX::XMVectorGetW(inputPlane);
            
    return winrt::Windows::Foundation::Numerics::plane(normal.x, normal.y, normal.z, distance);
}

winrt::Windows::Graphics::Holographic::HolographicStereoTransform SampleRemoteApp::ConvertPoseToStereoTransform(SampleRemoteApp::FilePose nextPose) {
    auto position = DirectX::XMFLOAT3(nextPose.x, nextPose.y, nextPose.z);
    auto orientation = DirectX::XMFLOAT4(nextPose.qx, nextPose.qy, nextPose.qz, nextPose.qw);
    ...
    winrt::Windows::Foundation::Numerics::float4x4 winrtMatrix;
    DirectX::XMStoreFloat4x4(reinterpret_cast<DirectX::XMFLOAT4X4*>(&winrtMatrix), viewProjectionMatrix);

    winrt::Windows::Graphics::Holographic::HolographicStereoTransform stereoTransform;
    stereoTransform.Left = winrtMatrix;
    stereoTransform.Right = winrtMatrix;
...
void SampleRemoteApp::OnKeyPress(char key)
{
    switch (key)
    {
        ...
        case 'c':
            ...
            m_spinningCubeRenderer1->TogglePauseState();
            m_spinningCubeRenderer2->TogglePauseState();
            for(int i = 0; i < 1100; ++i) {
                m_spinningCubeRendererVector[i]->TogglePauseState();
            }
            break;
...
void SampleRemoteApp::ParseLaunchArguments(std::wstring_view arguments)
{
    ...
    if (argCount < 2) {
        OutputDebugStringW(L"ERROR: Missing or not enough arguments.\nUsage: [flags] <ip> <trace_filepath>");
    ...
    for (int argIndex = 0; argIndex < argCount; ++argIndex)
    {
        ...
        if (arg[0] == '-')
        {
            ...
            if (param == L"noreplay")
            {
                options.render_replay = false;
                continue;
            }

            if (param == L"writepose")
            {
                if ((argIndex + 1 < argCount) && (argIndex + 3 < argCount))
                {
                    options.write_pose = true;
                    options.write_outputpose_file = args[argIndex + 1];
                    argIndex++;
                }
                else
                {
                    OutputDebugStringW(L"ERROR: Not enough arguments provided.");
        ...
        {
            options.poseFile = args[argIndex];
            std::ifstream file(options.poseFile);
            std::string line;
            while (std::getline(file, line)) {
                std::istringstream iss(line);
                float tstamp, x, y, z, qx, qy, qz, qw;
                if (!(iss >> tstamp >> x >> y >> z >> qx >> qy >> qz >> qw)) { break; }

                SampleRemoteApp::FilePose newPose = SampleRemoteApp::FilePose(tstamp, x, y, z, qx, qy, qz, qw);
                options.poses0.push(newPose);

                options.poses1.push(newPose);
                
                /*std::wostringstream debugMsg;
                debugMsg << "message has a" << 3 << " number.";
                OutputDebugStringW(debugMsg.str().c_str());
                */
            }
            options.initialPose = options.poses0.front();
        }
    }
...
void SampleRemoteApp::OnRecognizedSpeech(const winrt::hstring& recognizedText)
{
    ...
    if (changedColor && m_spinningCubeRenderer)
    {
        ...
        m_spinningCubeRenderer1->SetColorFilter(color);
        m_spinningCubeRenderer2->SetColorFilter(color);

        for(int i = 0; i < 1100; ++i) {
            m_spinningCubeRendererVector[i]->SetColorFilter(color);
        }
    }
}
...
HolographicFrame SampleRemoteApp::Update()
{
    ...
    try
    {
        ...
        else
        {
            ...
            initialCubePosition.z = -2;

            static float3 initialCubePosition1 = float3::zero();
            initialCubePosition1.z = -2;
            initialCubePosition1.y = 0.5;

            static float3 initialCubePosition2 = float3::zero();
            initialCubePosition2.z = -2;
            initialCubePosition2.y = -0.5;
            
            //get through the first frame
            if (m_options.firstTransform) {
                m_options.firstTransform = false;
                //m_spinningCubeRenderer->SetPosition(initialCubePosition);
                //m_spinningCubeRenderer1->SetPosition(initialCubePosition1);
                //m_spinningCubeRenderer2->SetPosition(initialCubePosition2);
                //delta pose starts off 0
            }
            else {
                winrt::Windows::Perception::Spatial::SpatialLocator locator = SpatialLocator::GetDefault();
                winrt::Windows::Perception::PerceptionTimestamp nowTimeStamp = winrt::Windows::Perception::PerceptionTimestampHelper::FromHistoricalTargetTime(winrt::clock::now());

                winrt::Windows::Perception::Spatial::SpatialLocation location = locator.TryLocateAtTimestamp(nowTimeStamp, coordinateSystem);
				if (location && m_options.flag == false) {
					float3 headPosition = location.Position();
					quaternion headOrientation = location.Orientation();

                    FilePose initialHeadPose = { 0, headPosition.x, headPosition.y, headPosition.z, headOrientation.x, headOrientation.y, headOrientation.z, headOrientation.w };
                    m_options.initialPose = m_options.poses0.front();
                    m_options.flag = true;

					// convert to quaternion
					auto initialHololensQuart = winrt::Windows::Foundation::Numerics::quaternion(headOrientation.x, headOrientation.y, headOrientation.z, headOrientation.w);
                    auto initialFileQuart = winrt::Windows::Foundation::Numerics::quaternion(m_options.initialPose.qx, m_options.initialPose.qy, m_options.initialPose.qz, m_options.initialPose.qw);

					//create rotation matrix from quaternions
					winrt::Windows::Foundation::Numerics::float4x4 initialHololensMatrix = winrt::Windows::Foundation::Numerics::make_float4x4_from_quaternion(initialHololensQuart);
					winrt::Windows::Foundation::Numerics::float4x4 initialFileMatrix = winrt::Windows::Foundation::Numerics::make_float4x4_from_quaternion(initialFileQuart);

                    // Do not need to calculate translation, hololens initializes at 0,0,0

					float4x4 invInitialRotation = float4x4();
					winrt::Windows::Foundation::Numerics::invert(initialHololensMatrix, &invInitialRotation);

                    float4x4 rotMatrix = invInitialRotation;
                    
					float4x4 Tt = rotMatrix;
					DirectX::XMMATRIX RotMatrixInv = DirectX::XMMATRIX(Tt.m11, Tt.m12, Tt.m13, Tt.m14,
						Tt.m21, Tt.m22, Tt.m23, Tt.m24,
						Tt.m31, Tt.m32, Tt.m33, Tt.m34,
						Tt.m41, Tt.m42, Tt.m43, Tt.m44);

                    m_spinningCubeRenderer->TransformPcube(RotMatrixInv);
                    m_spinningCubeRenderer1->SetPcube(initialCubePosition1);
                    m_spinningCubeRenderer1->TransformPcube(RotMatrixInv);
                    m_spinningCubeRenderer2->SetPcube(initialCubePosition2);
                    m_spinningCubeRenderer2->TransformPcube(RotMatrixInv);

                    int gridSize = 10;
                    float spacing = 2.0f;
                    float offset = (gridSize - 1) * spacing * 0.5f;  // Offset to center the grid at the origin
                    for(int k = 0; k < 11; ++k) {  // 11 layers
                        for(int i = 0; i < gridSize; ++i) {
                            for(int j = 0; j < gridSize; ++j) {
                                // Calculate the position of the cube
                                float3 initialCubePosition = float3(i * spacing - offset, k * spacing - 3*spacing, j * spacing - offset);

                                // Set the position of the cube
                                m_spinningCubeRendererVector[k * gridSize * gridSize + i * gridSize + j]->SetPcube(initialCubePosition);
                                m_spinningCubeRendererVector[k * gridSize * gridSize + i * gridSize + j]->TransformPcube(RotMatrixInv);
                            }
                        }
                    }

                    /*
                    // hallway case, spread cubes more horizontally
                    int width = 22;
                    int length = 10;
                    int height = 5;
                    float spacing = 3.0f;
                    float offsetW = (width - 1) * spacing * 0.5f;  // Offset to center the grid at the origin
                    float offsetL = (length - 1) * spacing * 0.5f;  // Offset to center the grid at the origin
                    float offsetH = (height - 1) * spacing * 0.5f;  // Offset to center the grid at the origin
                    for(int h = 0; h < height; ++h) {  
                        for(int l = 0; l < length; ++l) {
                            for(int w = 0; w < width; ++w) {
                                // Calculate the position of the cube
                                float3 initialCubePosition = float3(w * spacing - offsetW, h * spacing - offsetH, l * spacing - offsetL);

                                // Set the position of the cube
                                m_spinningCubeRendererVector[h * width * length + l * width + w]->SetPcube(initialCubePosition);
                                m_spinningCubeRendererVector[h * width * length + l * width + w]->TransformPcube(RotMatrixInv);
                            }
                        }
                    }
                    */
				...
				//begin record poses from hololens
                if (m_options.write_pose) {
                    winrt::Windows::Perception::Spatial::SpatialLocator locator = SpatialLocator::GetDefault();
                    winrt::Windows::Perception::PerceptionTimestamp nowTimeStamp = winrt::Windows::Perception::PerceptionTimestampHelper::FromHistoricalTargetTime(winrt::clock::now());
                    ...
                    if (location)
                    {
                        ...
                        float3 headPosition = location.Position();
                        quaternion headOrientation = location.Orientation();
                        std::wstring direcFile = m_options.write_outputpose_file;//L"C:/Users/cshu/Documents/shool_work/2023-2024/holo_remoting_scripts/Case1Tests/Test7_Replay.txt";

                        std::ofstream log;
                        log.open(direcFile, std::ofstream::app);

                        // Convert the timestamp to a long long representing the time in nanoseconds since Unix epoch
                        // Assuming dateTime is your DateTime object
                        auto duration = nowTimeStamp.TargetTime().time_since_epoch();
                        //long long timestamp = std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count();

                        log << duration.count() << " " << headPosition.x << " " << headPosition.y << " " << headPosition.z << " "
                            << headOrientation.x << " " << headOrientation.y << " " << headOrientation.z << " " << headOrientation.w << "\n";

                        log.close();
                    }
                }//end of logging pose to file
                
                //replay input file
			    if (m_options.flag && m_options.render_replay) {
				//if (false) {

					//get pose at time t
					FilePose nextPose = m_options.poses0.front();
					if (m_options.poses0.size() > 1) { m_options.poses0.pop(); }

					// convert to quaternion
					auto initialQuart = winrt::Windows::Foundation::Numerics::quaternion(m_options.initialPose.qx, m_options.initialPose.qy, m_options.initialPose.qz, m_options.initialPose.qw);
					auto nextQuart = winrt::Windows::Foundation::Numerics::quaternion(nextPose.qx, nextPose.qy, nextPose.qz, nextPose.qw);

					//create rotation matrix from quaternions
					winrt::Windows::Foundation::Numerics::float4x4 initMatrix = winrt::Windows::Foundation::Numerics::make_float4x4_from_quaternion(initialQuart);
					winrt::Windows::Foundation::Numerics::float4x4 nextMatrix = winrt::Windows::Foundation::Numerics::make_float4x4_from_quaternion(nextQuart);

					
					float dx = nextPose.x - m_options.initialPose.x;
					float dy = nextPose.y - m_options.initialPose.y;
					float dz = nextPose.z - m_options.initialPose.z;

                    // take inverse of initial trace matrix and apply to next frame trace matrix 
					float4x4 invInitialRotation = float4x4();
					winrt::Windows::Foundation::Numerics::invert(initMatrix, &invInitialRotation);
					float4x4 rotMatrix = nextMatrix * invInitialRotation;

                    //update tranlation component
					rotMatrix.m14 = -dx;
					rotMatrix.m24 = -dy;
					rotMatrix.m34 = -dz;
					float4x4 Tt = rotMatrix;

                    //converting from float4x4 to xmmatrix
					DirectX::XMMATRIX xmmatTinv = DirectX::XMMATRIX(Tt.m11, Tt.m12, Tt.m13, Tt.m14,
						Tt.m21, Tt.m22, Tt.m23, Tt.m24,
						Tt.m31, Tt.m32, Tt.m33, Tt.m34,
						Tt.m41, Tt.m42, Tt.m43, Tt.m44);

                    //updates cube position
					m_spinningCubeRenderer->UpdateTinv(xmmatTinv);
                    m_spinningCubeRenderer1->UpdateTinv(xmmatTinv);
                    m_spinningCubeRenderer2->UpdateTinv(xmmatTinv);
                    for(int i = 0; i < 1100; ++i) {
                        m_spinningCubeRendererVector[i]->UpdateTinv(xmmatTinv);
                    }
				}
        ...
        m_spinningCubeRenderer1->Update(timeSinceStart.count(), prediction.Timestamp(), coordinateSystem);
        m_spinningCubeRenderer2->Update(timeSinceStart.count(), prediction.Timestamp(), coordinateSystem);
        for(int i = 0; i < 1100; ++i) {
            m_spinningCubeRendererVector[i]->Update(timeSinceStart.count(), prediction.Timestamp(), coordinateSystem);
        }
                ...
                try
                {
                    ...
                    m_deviceResources->UseD3DDeviceContext([&](ID3D11DeviceContext3* context) {
                        ...
                        if (cameraActive)
                        {
                            ...
                            m_spinningCubeRenderer1->Render(pCameraResources->IsRenderingStereoscopic(), cullingFrustum);
                            m_spinningCubeRenderer2->Render(pCameraResources->IsRenderingStereoscopic(), cullingFrustum);
                            for(int i = 0; i < 1100; ++i) {
                                m_spinningCubeRendererVector[i]->Render(pCameraResources->IsRenderingStereoscopic(), cullingFrustum);
                            }
...
void SampleRemoteApp::CreateHolographicSpaceAndDeviceResources()
{
    ...
    m_spinningCubeRenderer1 = std::make_unique<SpinningCubeRenderer>(m_deviceResources);
    m_spinningCubeRenderer2 = std::make_unique<SpinningCubeRenderer>(m_deviceResources);

    for(int i = 0; i < 1100; ++i) {
        m_spinningCubeRendererVector.push_back(std::make_unique<SpinningCubeRenderer>(m_deviceResources));
    }
        ...
        if (store)
        {
            ...
            if (anchors.HasKey(L"position"))
            {
                ...
                if (positionToOrigin)
                {
                    ...
                    m_spinningCubeRenderer1->SetPosition(res);
                    m_spinningCubeRenderer2->SetPosition(res);
                    for(int i = 0; i < 1100; ++i) {
                        m_spinningCubeRendererVector[i]->SetPosition(res);
                    }
...
void SampleRemoteApp::OnDeviceLost()
{
    ...
    m_spinningCubeRenderer1->ReleaseDeviceDependentResources();
    m_spinningCubeRenderer2->ReleaseDeviceDependentResources();
    for(int i = 0; i < 1100; ++i) {
        m_spinningCubeRendererVector[i]->ReleaseDeviceDependentResources();
    }
...
void SampleRemoteApp::OnDeviceRestored()
{
    ...
    m_spinningCubeRenderer1->CreateDeviceDependentResources();
    m_spinningCubeRenderer2->CreateDeviceDependentResources();
    for(int i = 0; i < 1100; ++i) {
        m_spinningCubeRendererVector[i]->CreateDeviceDependentResources();
    }
